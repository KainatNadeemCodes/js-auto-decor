...
final class CEI_Core
{
	...
	static public function controls_enqueue_scripts()
	{
		...
		wp_localize_script('cei-js', 'CEIl10n', array(
			'emptyImport'	=> __('Please choose a file to import.', 'kata-plus')
		));
	...
	static public function register($wp_customize)
	{
		...
		$description .= __('Assistant makes tasks like this easier!', 'kata-plus');

		$description .= sprintf(
			'<br /><a href="%1$s" class="external-link" target="_blank">%2$s<span class="screen-reader-text"> %3$s</span></a>',
			esc_url(__('https://assistant.pro/')),
			__('Get started with a Free Account', 'kata-plus'),
			/* translators: Accessibility text. */
			__('(opens in a new tab)', 'kata-plus')
		);
		...
		$wp_customize->add_section('cei-section', array(
			'title'	   => __('Export/Import', 'kata-plus'),
			'priority' => 10000000,
	...
	static private function _export($wp_customize)
	{
		...
		$styler_option = $class->Export('kirki');

		$mods = get_theme_mods();
		$data = array(
			'template'       => $template,
			'mods'           => $mods ? $mods : array(),
			'styler_options' => $styler_option,
			'options'	     => array()
		);
	...
	static private function _import($wp_customize)
	{
		...
		if (! file_exists($file['file'])) {
			$cei_error = __('Error importing settings! Please try again.', 'kata-plus');
			return;
		}
		...
		if ('array' != gettype($data)) {
			$cei_error = __('Error importing settings! Please check that you uploaded a customizer export file.', 'kata-plus');
			return;
		}
		...
		if ($data['template'] != $template) {
			$cei_error = __('Error importing settings! The settings you uploaded are not for the current theme.', 'kata-plus');
			return;
		}
	 ...
	 * @return void
	 */
	static public function _custom_import($customizer_file, $cei_import_images = true)
	{
		// Make sure WordPress upload support is loaded.
		if (! function_exists('wp_handle_upload')) {
			require_once(ABSPATH . 'wp-admin/includes/file.php');
		...
		// Get the upload data.
		$raw = file_get_contents($customizer_file);
		...
		if (isset($data['styler_options'])) {

			foreach ($data['styler_options'] as $option_key => $option_value) {
				update_option('styler_' . $option_key, $option_value);
			}
		}
