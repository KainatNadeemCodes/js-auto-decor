...
if (! class_exists('Kata_Plus_Importer')) {
	class Kata_Plus_Importer
	{
		...
		public function definitions()
		{
			...
			self::$api_url        = self::$protocol . 'katademos.webnus.net/Demo';
		}
		...
		public function GoPro($demo_name, $screenshot, $is_pro)
		{
			if ($is_pro && ! class_exists('Kata_Plus_pro')) {
				$this->access_denied_step($demo_name, $screenshot, '<p style="margin-top: 0;">' . __('To import Pro & Fast demos you must buy the pro version', 'kata-plus') . '</p><div class="ktp-go-pro"><a href="' . esc_url('https://my.webnus.net/pricing/') . '" class="kata-btn"> ' . __('Buy Pro version', 'kata-plus') . '</a></div>');
				return false;
		 ...
		 * @since   1.0.0
		 */
		public function HeaderModal($name, $active_step = 0)
		{
			$active_step = (int) $active_step;
			if ($active_step < 0 || $active_step > 2) {
				$active_step = 0;
			}
			$import_finished = 2 === $active_step;
?>
			<ul class="kata-import-wizard-header">
				<li class="kt-importer-step kt-last-step<?php echo 2 === $active_step ? ' kt-active-step' : ''; ?>" data-step="2"> <a href="#"><span>3. </span><?php echo esc_html__('Import Progress', 'kata-plus'); ?></a> </li>
				<li class="kt-importer-step<?php echo 1 === $active_step ? ' kt-active-step' : ''; ?><?php echo $import_finished ? ' inactive' : ''; ?>" data-step="1"> <a href="#"><span>2. </span><?php echo esc_html__('Select Content', 'kata-plus'); ?></a> </li>
				<li class="kt-importer-step<?php echo 0 === $active_step ? ' kt-active-step' : ''; ?><?php echo $import_finished ? ' inactive' : ''; ?>" data-step="0"> <a href="#"> <span>1. </span><?php echo esc_html__('Install Plugins', 'kata-plus'); ?></a> </li>
			</ul>
										...
										<div class="kata-required-plugins">
											...
											if ($required_plugins_arr) {
												foreach ($required_plugins_arr as $slg) {
													$slg = TGM_Plugin_Activation::$instance->resolve_required_plugin_slug($slg);
											...
											if ($required_plugins) {
												foreach ($required_plugins as $plugin) {
													...
													$is_plugin_active_class     = TGM_Plugin_Activation::$instance->is_plugin_requirement_satisfied($plugin['slug']) ? ' active' : '';
											?>
		...
		public function Importer()
		{
			...
			// replace webnus.net with katademos.com
			foreach (glob($upload_dir . '*.xml') as $files) {
				$content = file_get_contents($files);
				$content = str_replace(
					array('webnus.net\/kata\/', 'webnus.net/kata/', 'webnus.net/kata-blog/', 'webnus.net\/kata-blog\/'),
			...
			switch ($import_item) {
				...
				case 'causes':
					...
					// Utils::replace_urls( 'https://webnus.net/kata-blog/' . $settings['demo'], get_site_url() );
					// Utils::replace_urls( 'https://webnus.net/kata/' . $settings['demo'], get_site_url() );
		...
		public function ImportDone()
		{
			...
			Utils::replace_urls('https://webnus.net/kata/' . $demo_slug, get_site_url());
			Utils::replace_urls('https://webnus.net/kata/fast' . $demo_slug, get_site_url());
			Utils::replace_urls('https://webnus.net/kata/free' . $demo_slug, get_site_url());
			Utils::replace_urls('https://webnus.net/kata-blog/' . $demo_slug, get_site_url());
			Utils::replace_urls('https://webnus.net/kata-pro/' . $demo_slug, get_site_url());
			...
			<?php if ($status === true) : ?>
				...
				<div class="kata-lightbox-contents">
					<div class="kata-lightbox">
						...
						self::HeaderModal($modal_title, 2);
						?>
						<div class="kata-lightbox-content kata-importer-settings">
							<div class="kata-col-import-demo-image">
								<img src="<?php echo esc_url($demo_data->data->images[0]); ?>" alt="<?php echo esc_attr($demo_data->data->name); ?>">
			...
			wp_localize_script(
				...
				array(
					'translation' => array(
						...
						'fail-plugin-timeout'  => esc_html__( 'The request took too long (large plugins need more time). Please try again or install the plugin manually under Plugins.', 'kata-plus' ),
						'plugin-error-title'   => esc_html__( 'Plugin action failed', 'kata-plus' ),
					),
				)
			);
		}
