	...
	private function convert_to_css( $properties ) {
		...
		foreach ( $properties as $property => $data ) {
			...
			if ( 'content' === $property ) {
				...
				// Check if value starts with backslash
				$first_char = strlen( $normalized_value ) > 0 ? $normalized_value[0] : '';
				$has_backslash = ( $first_char === '\\' );
				
				if ( $has_backslash ) {
					// Extract hex part after backslash
					$hex_part = substr( $normalized_value, 1 );
					
					// Check if it's valid hex
					if ( preg_match( '/^[0-9A-Fa-f]{1,6}$/i', $hex_part ) ) {
						// Preserve the backslash format, just uppercase the hex
						$hex_part = strtoupper( $hex_part );
						$normalized_value = '\\' . $hex_part;
					} else {
						// Not hex, preserve as is but escape quotes
						$normalized_value = str_replace( '"', '\\"', $normalized_value );
					}
				} elseif ( preg_match( '/^[0-9A-Fa-f]{1,6}$/i', $normalized_value ) ) {
					// Value is hex without backslash, add backslash
					$hex_candidate = strtoupper( $normalized_value );
					
					if ( strlen( $hex_candidate ) < 4 ) {
						$hex_candidate = str_pad( $hex_candidate, 4, '0', STR_PAD_LEFT );
					}
					$normalized_value = '\\' . $hex_candidate;
				} else {
					// For non-hex values, only escape quotes, don't touch backslashes
					$normalized_value = str_replace( '"', '\\"', $normalized_value );
				}
