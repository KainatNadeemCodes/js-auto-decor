...
class DemoConvertorWebService
{
	...
	private function __construct()
	{
		...
		if (isset($_REQUEST['update-customizer'])) {
			// Move the update-customizer handler to init hook to ensure WordPress is fully loaded
			add_action('init', array($this, 'handle_update_customizer'), 10);
		}
		// Private to prevent instantiation
	}

	/**
	 * Handle update-customizer request
	 * This is now in a separate method to allow hooking into init
	 */
	public function handle_update_customizer()
	{
		...
		// Validate input data
		if (!is_array($customizer_data) || !isset($customizer_data['data']) || !is_string($customizer_data['data'])) {
			wp_send_json_error('Invalid input data format', 400);
			exit;
		}

		// Security: Validate that this is a safe serialized string
		if (preg_match('/[CO]:[+\-]?[0-9]+:"/', $customizer_data['data'])) {
			wp_send_json_error('Security validation failed: unsafe data format', 400);
			exit;
		}
		...
		if (!is_array($customizer_data)) {
			wp_send_json_error('Invalid data structure after unserialization', 400);
			exit;
		}
		...
		foreach ($customizer_data as $key => $value) {
			...
			foreach ($value as $id => $styler) {
				if (is_array($styler) || is_object($styler)) {
					$styler = str_replace(
						array('webnus.net\/kata\/', 'webnus.net/kata/', 'webnus.net/kata-blog/', 'webnus.net\/kata-blog\/', 'webnus.net\\\/kata-blog', 'webnus.net\\\/kata'),
	...
	public function convertAndSaveData()
	{
		...
		if (is_serialized($templateData)) {
			...
			if (is_string($templateData) && preg_match('/[CO]:[+\-]?[0-9]+:"/', $templateData)) {
				wp_send_json_error('Security validation failed: Unsafe data detected', 400);
				exit;
			}
