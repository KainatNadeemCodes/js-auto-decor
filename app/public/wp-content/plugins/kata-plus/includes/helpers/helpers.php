...
if ( ! class_exists( 'Kata_Plus_Helpers' ) ) {
	...
	class Kata_Plus_Helpers {
		...
		public function actions() {
			...
			add_filter( 'elementor/widget/render_content', array( $this, 'strip_lazyload_from_elementor_widget' ), 10, 2 );
		...
		/**
		 * Whether Kata lazyload is enabled in theme options.
		 *
		 * @since 1.0.0
		 * @return bool
		 */
		public static function is_lazyload_enabled() {
			$value = get_theme_mod( 'kata_plus_pro_lazyload', '0' );
			return filter_var( $value, FILTER_VALIDATE_BOOLEAN ) || '1' === (string) $value;
		}

		/**
		 * Return kata-lazyload class when the option is enabled.
		 *
		 * @since 1.0.0
		 * @param string $classes Optional extra classes.
		 * @return string
		 */
		public static function lazyload_class( $classes = '' ) {
			if ( ! self::is_lazyload_enabled() ) {
				return trim( $classes );
			}
			$classes = trim( $classes );
			return $classes ? $classes . ' kata-lazyload' : 'kata-lazyload';
		}

		/**
		 * Remove kata-lazyload markup when lazyload is disabled.
		 *
		 * @since 1.0.0
		 * @param string $html HTML content.
		 * @return string
		 */
		public static function strip_lazyload_markup( $html ) {
			if ( self::is_lazyload_enabled() || empty( $html ) ) {
				return $html;
			}
			return preg_replace( '/\s*kata-lazyload\b/', '', $html );
		}

		/**
		 * Strip lazyload class from Elementor widget output when disabled.
		 *
		 * @since 1.0.0
		 * @param string $content Widget HTML.
		 * @return string
		 */
		public function strip_lazyload_from_elementor_widget( $content ) {
			return self::strip_lazyload_markup( $content );
		}
		...
		public static function get_attachment_image_html( $settings, $image_size_key = 'image', $retina = '', $class = '' ) {
			...
			} else if ( 'custom' !== $settings[ $image_size_key . '_size' ] && $lazyload && ! isset( $_GET['action'] ) ) {
