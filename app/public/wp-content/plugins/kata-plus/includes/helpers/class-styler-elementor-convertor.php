...
class StylerElementorDataConverter
{
	...
	public function convertAndSaveData()
	{
		...
		if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'kata_update_db_fetch_data' && isset($_REQUEST['new_to_update_id'])) {
			...
			if (! isset($_REQUEST['nonce']) || ! wp_verify_nonce($_REQUEST['nonce'], 'kata_update_nonce')) {
			...
			$post_id = sanitize_text_field($_REQUEST['new_to_update_id']);
			...
			switch ($post_id) {
				case 'update_kirki':
					try {
						...
						wp_send_json_success(array(
							'message' => 'Kirki settings updated successfully',
							'task_id' => $post_id
						));
					} catch (Exception $e) {
						wp_send_json_error(array(
							'message' => 'Failed to update Kirki settings: ' . $e->getMessage(),
							'code' => 'kirki_update_failed'
						));
					}
					break;
				...
				case 'regenerate_elementor_data':
					try {
						if (class_exists('\Elementor\Plugin')) {
							...
							wp_send_json_success(array(
								'message' => 'Elementor cache cleared and CSS regenerated successfully',
								'task_id' => $post_id
							));
						} else {
							wp_send_json_error(array(
								'message' => 'Elementor plugin is not active or available',
								'code' => 'elementor_not_available'
							));
						}
					} catch (Exception $e) {
						wp_send_json_error(array(
							'message' => 'Failed to clear Elementor cache: ' . $e->getMessage(),
				...
				default:
					...
					// Check if post is already updated
					if (get_post_meta($post_id, '_elementor_data_updated_1.3', true)) {
						wp_send_json_success(array(
							'message' => "Post '{$post->post_title}' is already updated",
							'task_id' => $post_id,
					...
					if (! empty($old_data)) {
						// Check if the data contains styler
						if (strpos($old_data, 'styler') !== false) {
							try {
								...
								wp_send_json_error(array(
									'message' => "Failed to update post '{$post->post_title}': " . $e->getMessage(),
									'code' => 'post_update_failed'
								));
							}
						} else {
							wp_send_json_success(array(
								'message' => "Post '{$post->post_title}' does not contain styler data, skipping",
								'task_id' => $post_id,
								'skipped' => true
							));
						}
						...
						wp_send_json_success(array(
							'message' => "Post '{$post->post_title}' has no Elementor data, skipping",
							'task_id' => $post_id,
							'skipped' => true
						));
					}
