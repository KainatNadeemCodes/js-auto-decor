...
<div class="kata-postmetadata">
	...
	if ($settings['categories']) {
		...
		if (get_post_type() == 'post' || get_post_type() == 'kata_plus_builder') {
			$taxonomy = 'category';
		} elseif (get_post_type() == 'kata_grid') {
		...
		if (isset($post_categories)) {
			foreach ($post_categories as $post_category) {
				/* translators: %s is replaced width $post_category->name */
	...
	if ($settings['tags']) {
		...
		if ($tags_list) {
			echo '<span class="kata-tags-links">
				' . Kata_Plus_Helpers::get_icon('', $settings['tags_icon']) . ' ' . wp_kses_post($tags_list) . '
			</span>';
		}
	}
	...
	if ($settings['date']) {
		...
		$posted_on = is_single() ? $time_string : '<a href="' . esc_url(get_permalink($latest_post_id)) . '" rel="bookmark">' . $time_string . '</a>';

		$kata_icon_html = Kata_Plus_Helpers::get_icon('', $settings['date_icon'], 'df-fill');
		// Remove all inline fill attributes from SVG elements to allow CSS-based coloring via Elementor styler.
		$kata_icon_html = preg_replace('/\sfill=("|\')[^"\']*\1/i', '', $kata_icon_html);

		echo '<span class="kata-post-date">' . $kata_icon_html . $posted_on . '</span>'; // WPCS: XSS OK.
	}
	...
	if ($settings['author']) {
		...
		echo '<span class="kata-post-author">
				' . $post_author_avatar .
			'<a href="' . esc_url(get_author_posts_url($post_author_id)) . '">
...
<style>
	/* Ensure SVG icons inherit color and size from parent properly */
	.elementor-element-<?php echo esc_attr($this->get_id()); ?>.kata-postmetadata .kata-icon svg {
		fill: currentColor;
		width: 1em;
		height: 1em;
