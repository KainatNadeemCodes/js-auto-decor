...
class Kata_Plus_Counter extends Widget_Base {
	...
	public function get_style_depends() {
		Kata_Plus_Register_Assets::register_elementor_counter_style();

		return array( 'kata-plus-counter', 'widget-counter' );
	}

	public function get_script_depends() {
		return array( 'jquery-numerator', 'kata-plus-counter' );
	}

	/**
	 * Value to animate from (not the initial on-screen number).
	 *
	 * @param float|int|string $ending_number Target number.
	 * @param float|int|string $starting_number Optional panel value.
	 * @return float
	 */
	public static function get_animation_from_number( $ending_number, $starting_number = '' ) {
		$ending = is_numeric( $ending_number ) ? (float) $ending_number : 0;

		if ( is_numeric( $starting_number ) && (float) $starting_number > 0 && (float) $starting_number < $ending ) {
			return (float) $starting_number;
		}
		...
		return max( 0, floor( $ending * 0.375 ) );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'starting_number',
			array(
				'label'       => __( 'Animation Start Number', 'kata-plus' ),
				'type'        => Controls_Manager::NUMBER,
				'default'     => '',
				'description' => __( 'Number to animate from when the counter enters the viewport. The ending number is shown first. Leave empty to auto-calculate (about 37% of the ending value).', 'kata-plus' ),
			)
		);
		$this->add_control(
			'ending_number',
