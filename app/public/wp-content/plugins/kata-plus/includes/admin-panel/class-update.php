...
class UpdatePanelForNewStyler {
	...
	public function redirect() {
		...
		if ( ! isset( $_REQUEST['action'] ) || $_REQUEST['action'] !== 'kata_update_db_initial_ids' ) {
			...
			wp_safe_redirect( admin_url( 'admin.php?page=kata-plus-update' ) );
			exit;
		}
	}
	...
	public function initial_ids() {
				...
				if ( ! empty( $old_data ) ) {
					...
					if ( strpos( $old_data, 'styler' ) !== false ) {
						if ( ! get_post_meta( $post_id, '_elementor_data_updated_1.3', true ) ) {
							...
							$ids[ $post_id ] = ! empty( $post_title ) ? $post_title : "Post ID: {$post_id}";
						}
					}
				}
			}
		...
		if ( ! $term_meta_list && ! isset( $term_meta_list['updated'] ) ) {
			$ids['update_kirki'] = 'Update Kirki Settings';
			update_option( 'kirki_migration_list', $term_meta_list );
		}

		$ids['regenerate_elementor_data'] = 'Regenerate CSS & Data Files';
		...
		wp_send_json_success(
			array(
				'ids' => $ids,
				'total_tasks' => count( $ids ),
				'message' => 'Update tasks retrieved successfully'
			)
		);
	}
} //Class
