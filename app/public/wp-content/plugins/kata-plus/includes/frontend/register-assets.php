...
class Kata_Plus_Register_Assets {
	...
	private static function hooks() {
		...
		add_action( 'elementor/frontend/after_register_styles', array( __CLASS__, 'register_elementor_counter_style' ) );
		add_action( 'elementor/editor/after_enqueue_styles', array( __CLASS__, 'register_elementor_counter_style' ), 5 );
	}
	...
	private static function get_scripts_list() {
		...
		$scripts = array(
			...
			'kata-plus-counter'                  => $assets . 'js/frontend/counter.js',
	...
	/**
	 * Register Elementor counter widget CSS for editor + frontend (before widget deps resolve).
	 *
	 * @since 1.5.0
	 * @return void
	 */
	public static function register_elementor_counter_style() {
		if ( wp_style_is( 'widget-counter', 'registered' ) || ! defined( 'ELEMENTOR__FILE__' ) ) {
			return;
		}
		...
		wp_register_style(
			'widget-counter',
			plugins_url( 'assets/css/widget-counter' . $suffix . '.min.css', ELEMENTOR__FILE__ ),
			array( 'elementor-frontend' ),
			defined( 'ELEMENTOR_VERSION' ) ? ELEMENTOR_VERSION : Kata_Plus::$version
		);
	}
	...
	public static function register_scripts() {
		self::register_elementor_counter_style();

		$scripts = self::get_scripts_list();
		$styles  = self::get_styles_list();
		$version = Kata_Plus::$version;

		if ( ! wp_script_is( 'jquery-numerator', 'registered' ) && defined( 'ELEMENTOR__FILE__' ) ) {
			wp_register_script(
				'jquery-numerator',
				plugins_url( 'assets/lib/jquery-numerator/jquery-numerator.min.js', ELEMENTOR__FILE__ ),
				array( 'jquery' ),
				'0.2.1',
				true
			);
		...
		foreach ( $scripts as $handle => $script ) {
			...
			if ( 'kata-plus-counter' === $handle && wp_script_is( 'jquery-numerator', 'registered' ) ) {
				$deps[] = 'jquery-numerator';
			}
