 ...
 * Plugin URI:      https://webnus.net/kata/
...
if ( ! class_exists( 'Kata_Plus' ) ) {
	class Kata_Plus {
		...
		public function definitions() {
			...
			self::$version                  = '1.6.2';
			self::$slug                     = 'kata-plus';
			self::$plugin_url               = 'https://webnus.net/kata/';
		...
		public function dependencies() {
			...
			// licensing.
			Kata_Plus_Autoloader::load( self::$dir . 'includes/licensing', 'class-kata-plus-license-activation' );
			...
			new Kata_Plus_License_Activation( self::$version, self::$slug, self::$name );
		}
		...
		public function plugin_row_meta( $plugin_meta, $plugin_file ) {
			if ( self::$name === $plugin_file ) {
				$row_meta = array(
					'docs'      => '<a href="https://webnus.net/kata/documentation/" aria-label="' . esc_attr( esc_html__( 'View Kata Documentation', 'kata-plus' ) ) . '" target="_blank">' . esc_html__( 'Docs & FAQs', 'kata-plus' ) . '</a>',
					'changes'   => '<a href="https://webnus.net/kata/documentation/changelog/" aria-label="' . esc_attr( esc_html__( 'View Kata Changelog', 'kata-plus' ) ) . '" target="_blank">' . esc_html__( 'Changelog', 'kata-plus' ) . '</a>',
					'pro'       => '<a href="https://my.webnus.net/pricing/" aria-label="' . esc_attr( esc_html__( 'View Kata Changelog', 'kata-plus' ) ) . '" target="_blank" style="color: red;">' . esc_html__( 'Go Pro', 'kata-plus' ) . '</a>',
					'shoppress' => '<a href="https://webnus.net/shoppress/" aria-label="' . esc_attr( esc_html__( 'ShopPress', 'kata-plus' ) ) . '" target="_blank" style="color: #837AF5;">' . esc_html__( 'ShopPress', 'kata-plus' ) . '</a>',
				);
			...
			foreach ( $fonts as $font ) {
				if ( isset( $font['source'] ) ) {
					...
					if ( $font['source'] === 'upload-font' ) {
						$font_names = json_decode( $font['name'], true );
						$font_names_list = [];
						if ( is_array( $font_names ) ) {
