...
(function ($) {
	...
	function kataParsePluginHtmlError(raw) {
		...
		return $.trim($doc.find('.wp-die-message').text());
	}
	...
	function kataGetPluginInstallBox($btn) {
		var $box = $btn.closest('.kata-lightbox').find('.kata-install-plugins').first();
		if (!$box.length) {
			$box = $btn.closest('.kata-col-import-demo').find('.kata-install-plugins').first();
		}
		return $box;
	}
	...
	function kataShowPluginNotice($btn, message) {
		...
		var $notice = $('<div class="kata-plugin-install-notice notice notice-error" role="alert"></div>');
		if (pluginName) {
			$notice.append(
		...
		$notice
			.append($('<p class="kata-plugin-install-notice-message"></p>').text(message))
			.appendTo($box);
