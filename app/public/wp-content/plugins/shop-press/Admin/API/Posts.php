...
class Posts {
	...
	public function get_post( $request ) {
		...
		if ( ! $post || ! is_a( $post, 'WP_Post' ) ) {
			return new \WP_REST_Response(
				array( 'code' => 'post_not_found', 'message' => __( 'Post not found.', 'shop-press' ) ),
				404
			);
		}
		...
		if ( ! current_user_can( 'read_post', $post_id ) ) {
			return new \WP_REST_Response(
				array( 'code' => 'rest_forbidden', 'message' => __( 'You cannot view this post.', 'shop-press' ) ),
				403
			);
		}
		...
		if ( ! is_wp_error( $post ) ) {
			...
			// Fallback: ensure FBT meta_fields when Pro filter may not have run.
			if ( 'shoppress_fbt' === $post_type && empty( $response['meta_fields'] ) ) {
				if ( class_exists( '\ShopPressPro\Modules\FrequentlyBoughtTogether\Main' ) && method_exists( '\ShopPressPro\Modules\FrequentlyBoughtTogether\Main', 'add_meta_fields' ) ) {
					$response = \ShopPressPro\Modules\FrequentlyBoughtTogether\Main::add_meta_fields( $response, $post_id, $post_type );
				}
			}
	...
	/**
	 * Ensure product_brand taxonomy is available (WooCommerce / Pro Brands module).
	 *
	 * @since 1.3.1
	 */
	private function ensure_brands_taxonomy_registered() {
		if ( taxonomy_exists( 'product_brand' ) ) {
			return;
		}
		$pro_path  = $this->get_pro_path();
		$brand_file = $pro_path ? $pro_path . 'Modules/Brands/Brand.php' : '';
		if ( $brand_file && file_exists( $brand_file ) ) {
			require_once $brand_file;
		}
		// product_brand is registered by WooCommerce - no separate registration needed.
	}

	/**
	 * Get ShopPress Pro plugin path.
	 *
	 * @return string
	 */
	private function get_pro_path() {
		if ( defined( 'SHOPPRESS_PRO_PATH' ) ) {
			return SHOPPRESS_PRO_PATH;
		}
	...
	public function get_terms( $request ) {
		...
		$this->ensure_brands_taxonomy_registered();
	...
	public function get_term( $request ) {
		...
		if ( ! is_wp_error( $term ) ) {
			...
			if ( 'product_brand' === $taxonomy && empty( $response['term_meta_fields'] ) && class_exists( 'ShopPressPro\Modules\Brands\Brand' ) ) {
				$response = \ShopPressPro\Modules\Brands\Brand::add_brands_meta_fields( $response, $term_id, $taxonomy );
			}
