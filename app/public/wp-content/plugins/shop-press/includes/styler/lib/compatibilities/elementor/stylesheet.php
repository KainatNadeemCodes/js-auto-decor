...
class StyleSheet {
	...
	/**
	 * Sanitize CSS before printing inside a <style> tag.
	 *
	 * This removes sequences that could break out of the style context
	 * and start a <script> block.
	 *
	...
	private function sanitize_css( $css ) {
		...
		// Strip sequences that can terminate the <style> tag or start scripts.
		$dangerous = array(
			'</style',
			'<script',
			'</script',
		...
		return str_ireplace( $dangerous, '', $css );
	}
	...
	/**
	 * Get elementor style dependencies (only include if registered).
	 *
	 * @since 1.0.0
	 *
	 * @return array
	 */
	private function get_elementor_style_deps() {
		return wp_style_is( 'elementor-frontend', 'registered' ) ? array( 'elementor-frontend' ) : array();
	}
	...
	public function enqueue_template_manager_style( $id ) {
		if ( ! is_preview() && ! Plugin::$instance->preview->is_preview_mode() ) {
			if ( realpath( implode( DIRECTORY_SEPARATOR, array( get_styler_upload_path(), 'elementor', 'css', "post-{$id}.css" ) ) ) ) {
				wp_enqueue_style( 'styler-post-' . $id, implode( '/', array( get_styler_upload_url(), 'elementor', 'css', "post-{$id}.css" ) ), $this->get_elementor_style_deps() );
			}
		}
	}
	...
	public function enqueue_style( $document = false ) {
		...
		if ( ! is_preview() && ! Plugin::$instance->preview->is_preview_mode() ) {
			if ( realpath( implode( DIRECTORY_SEPARATOR, array( get_styler_upload_path(), 'elementor', 'css', "post-{$id}.css" ) ) ) ) {
				wp_enqueue_style( 'styler-post-' . $id, $url, $this->get_elementor_style_deps(), null );
			}
	...
	public function enqueue_inline_styles( $id = false, $force = false ) {
		...
		if ( is_preview() || Plugin::$instance->preview->is_preview_mode() || $force ) {
			...
			foreach ( $styler_object as $key => $value ) {
				...
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS output sanitized by sanitize_css().
				printf( '<style id="%1$s">%2$s</style>', esc_attr( 'styler-' . $key ), $css );
			}
		}
	}
