...
class ShopPress_Widget_Layered_Nav extends \WC_Widget_Layered_Nav {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$script = "
				jQuery( function( $ ) {
					...
					$( document ).on( 'change', '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "', function() {
						var slug = $( this ).val();
	...
	protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
		...
		foreach ( $terms as $term ) {
			...
			$current_filter = isset( $_GET[ $filter_name ] ) ? explode( ',', wc_clean( sanitize_text_field( wp_unslash( $_GET[ $filter_name ] ) ) ) ) : array(); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
