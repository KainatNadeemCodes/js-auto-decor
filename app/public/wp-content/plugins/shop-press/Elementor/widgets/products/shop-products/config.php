...
class ShopProducts extends ProductsLoop {
	...
	protected function register_controls() {
		...
		ControlsWidgets::select2(
			...
			$this,
			false,
			/* translators: %s: link to products loop component */
			! $product_loop_status ? sprintf( esc_html__( 'To display the custom template in the front, you need to activate the %s component', 'shop-press' ), '<a href="' . esc_url( admin_url( 'admin.php?page=shoppress&sub=templates' ) ) . '" target="_blank">' . esc_html__( 'products loop', 'shop-press' ) . '</a>' ) : ''
		);
	...
	public function render() {
		...
		if ( $this->is_editor() ) {
			...
			<?php if ( $settings['result_count'] ) : ?>
				<p class="woocommerce-result-count"><?php echo esc_html( __( 'Showing all 12 results', 'shop-press' ) ); ?></p>
			<?php endif; ?>

			<?php if ( $settings['catalog_ordering'] ) : ?>
				<form class="woocommerce-ordering" method="get">
					<select name="orderby" class="orderby" aria-label="<?php esc_attr_e( 'Shop order', 'shop-press' ); ?>">
							<option value="menu_order" selected="selected"><?php echo esc_html( __( 'Default sorting', 'shop-press' ) ); ?></option>
							<option value="popularity"><?php echo esc_html( __( 'Sort by popularity', 'shop-press' ) ); ?></option>
							<option value="rating"><?php echo esc_html( __( 'Sort by average rating', 'shop-press' ) ); ?></option>
							<option value="date"><?php echo esc_html( __( 'Sort by latest', 'shop-press' ) ); ?></option>
							<option value="price"><?php echo esc_html( __( 'Sort by price: low to high', 'shop-press' ) ); ?></option>
							<option value="price-desc"><?php echo esc_html( __( 'Sort by price: high to low', 'shop-press' ) ); ?></option>
					</select>
