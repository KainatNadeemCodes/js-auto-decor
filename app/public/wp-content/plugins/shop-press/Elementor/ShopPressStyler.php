...
abstract class ShopPressStyler extends Widget_Base {
	...
	public function is_editor() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Preview flag for editor detection, no form processing.
		return Plugin::$instance->editor->is_edit_mode()
			||
			( in_array( get_post_type(), array( 'shoppress_pages', 'shoppress_loop' ), true ) && isset( $_GET['preview'] ) );
	}
	...
	public function register_group_styler( $sec_id, $sec_label, $fields, $condition = array() ) {
		...
		foreach ( $fields as $id => $item ) {
			$base_name   = ( $sec_id === $id && 1 === count( $fields ) ) ? 'styler_' . $id : 'styler_' . $sec_id . '_' . $id;
			$control_name = $base_name;
			$controls    = $this->get_controls();
			$counter     = 1;
			while ( isset( $controls[ $control_name ] ) ) {
				$control_name = $base_name . '_' . $counter;
				++$counter;
			}
