...
abstract class ShopPressWidgets extends ShopPressStyler {
	/**
	 * Enqueue the widget's declared style/script dependencies at render time.
	 *
	 * Elementor normally enqueues a widget's get_style_depends()/get_script_depends()
	 * automatically, but in some frontend render contexts ( e.g. widgets rendered
	 * inside header/footer or other templates built outside the main loop, and
	 * under the asset-loading behaviour of newer Elementor versions ) those
	 * dependencies are not printed - leaving the widget unstyled on the frontend
	 * while it still looks correct in the editor, where all widget assets load.
	 *
	 * Enqueuing the already-registered handles here is idempotent ( a no-op when
	 * Elementor has already enqueued them ) and guarantees the assets load
	 * wherever the widget actually renders.
	 *
	 * @since 1.5.1
	 */
	public function before_render() {
		foreach ( (array) $this->get_style_depends() as $style_handle ) {
			wp_enqueue_style( $style_handle );
		}

		foreach ( (array) $this->get_script_depends() as $script_handle ) {
