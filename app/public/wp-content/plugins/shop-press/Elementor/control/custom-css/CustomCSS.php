...
class ShopPressElementorCustomCSS extends Base_Data_Control {
	...
	public function enqueue() {
		// Use WordPress core CodeMirror editor.
		...
		// Enqueue custom script to initialize CodeMirror for Elementor controls.
		if ( false !== $editor_settings ) {
			wp_enqueue_script(
				'shoppress-codemirror-init',
				SHOPPRESS_URL . 'public/admin/elementor/codemirror-init.js',
				array( 'jquery', 'code-editor', 'elementor-editor' ),
				SHOPPRESS_VERSION,
				true
			);
		}
...
function shoppress_module_custom_css_editor( $custom_css ) {
	if ( ! empty( $custom_css ) && Plugin::$instance->editor->is_edit_mode() ) {
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS from trusted editor.
