...
class EditorCondition {
	...
	private static function is_loop() {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Elementor verifies nonce in its AJAX handler.
		return (
			( false === self::is_edit_mode() || ( isset( $_POST['action'] ) && 'elementor_ajax' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ) ) &&
	...
	private static function is_compare() {
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Action used for conditional check only; Compare AJAX verifies nonce separately.
		$action = isset( $_REQUEST['action'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['action'] ) ) : false;
		return self::check_custom_type( 'compare' ) || is_sp_compare_page() || in_array( $action, array( 'AddCompare', 'RemoveCompare' ), true );
	}
