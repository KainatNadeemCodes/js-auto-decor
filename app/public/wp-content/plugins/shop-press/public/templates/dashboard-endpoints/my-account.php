...
if ( ! is_user_logged_in() || isset( $wp->query_vars['lost-password'] ) ) {
	$sp_message = apply_filters( 'woocommerce_my_account_message', '' );

	if ( ! empty( $sp_message ) ) {
		wc_add_notice( $sp_message );
	}

	// After password reset, add confirmation message.
	if ( ! empty( $_GET['password-reset'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- WordPress password reset flow.
