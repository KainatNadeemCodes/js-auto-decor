...
if ( is_shop() || is_archive() ) {
	echo '<div class="sp-sidebar sp-product-filters ' . esc_attr( $drawer_classes ) . '">';
	...
	if ( $display_drawer ) {
		...
		echo '<div class="sp-drawer-click sp-open-product-filters-drawer">'
				. sp_kses_post( $btn_html ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post().
			. '</div>'
			. '<div class="sp-drawer-content-wrap ' . esc_attr( $mobile_drawer_content_classes ) . '">'
				. '<div class="sp-drawer-close">' . wp_kses( $close_icon, sp_allowd_svg_tags() ) . '</div>'
				. '<div class="sp-drawer-content-title">' . esc_html( $drawer_title ) . '</div>';
	}

	$show_clear_all = 'yes' === ( $args['show_clear_all'] ?? false );
	if ( $show_clear_all ) {

		echo '<button class="sp-reset-filters">' . esc_html( __( 'Clear All', 'shop-press' ) ) . '</button>';
	}
