...
if ( ! $registration_enabled && $checkout->is_registration_required() && ! is_user_logged_in() && ! $show_login ) {
	echo esc_html( apply_filters( 'woocommerce_checkout_must_be_logged_in_message', __( 'You must be logged in to checkout.', 'shop-press' ) ) );
	return;
}
...
<div class="sp-multistep-checkout">
	<div class="sp-tabs-wrapper">
		...
		<?php if ( $display_billing_and_shipping_steps_together ) : ?>
			<li class="sp-tab-item sp-billing-and-shipping <?php echo is_user_logged_in() ? esc_attr( 'current' ) : ''; ?>" data-step-title="billing-shipping">
				<span class="sp-tab-number"><?php echo esc_html( (string) current( $numbers ) ); ?></span>
				<span class="sp-tab-text"><?php esc_html_e( 'Billing & Shipping', 'shop-press' ); ?></span>
			</li>
		<?php else : ?>
			<li class="sp-tab-item sp-billing <?php echo is_user_logged_in() ? esc_attr( 'current' ) : ''; ?>" data-step-title="billing">
				<span class="sp-tab-number"><?php echo esc_html( (string) current( $numbers ) ); ?></span>
				<span class="sp-tab-text"><?php esc_html_e( 'Billing', 'shop-press' ); ?></span>
			</li>

			<li class="sp-tab-item sp-shipping" data-step-title="shipping">
				<span class="sp-tab-number"><?php echo esc_html( (string) next( $numbers ) ); ?></span>
	...
	<div class="sp-nav-wrapper">
		<?php if ( wc_get_page_id( 'shop' ) > 0 && $show_return_to_shop ) : ?>
			<a id="sp-return-to-shop" class="current sp-nav-button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
				<span class="woo-checkout-btn-icon left"><?php echo sp_kses_post( $arrow ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?></span>
		...
		<?php if ( $show_login ) : ?>
			<button id="sp-next" class="sp-nav-button">
				...
				<span class="woo-checkout-btn-icon right"><?php echo sp_kses_post( $arrow ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?></span>
			</button>
		<?php else : ?>
