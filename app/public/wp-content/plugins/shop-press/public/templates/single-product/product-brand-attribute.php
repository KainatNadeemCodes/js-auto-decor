...
<div class="sp-brands sp-brands-attrs sp-brands-grid sp-single-brands">
	...
	<div class="sp-brands-items">
		...
		foreach ( $attributes as $attribute ) {
			// Some attributes may not be taxonomy-based or may not resolve to a WC attribute object.
			if ( ! method_exists( $attribute, 'get_id' ) ) {
				continue;
			}

			$attribute_data = wc_get_attribute( $attribute->get_id() );

			if ( ! $attribute_data || ! isset( $attribute_data->type ) ) {
				continue;
			}
			...
			if ( $attribute_data->type === 'brand' ) {
				...
				echo wp_kses_post( Frontend::get_brand_attributes_output( $terms, $attribute_data->slug, $display_name, $display_logo ) );
