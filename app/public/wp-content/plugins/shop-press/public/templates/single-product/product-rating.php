		...
		<div class="woocommerce-product-rating sp-single-rating sp-modern-rating">
			...
			<?php if ( comments_open() ) : ?>
				<?php if ( 'yes' === $args['show_review_counter'] ) { ?>
					<?php /* translators: %s: review count HTML */ ?>
					<a href="<?php echo esc_url( $link ); ?>" class="woocommerce-review-link" rel="nofollow"><?php echo wp_kses_post( sprintf( _n( '%s review', '%s reviews', $review_count, 'shop-press' ), '<span class="count">' . esc_html( $review_count ) . '</span>' ) ); ?></a>
				<?php } ?>
			<?php endif ?>
		</div>
