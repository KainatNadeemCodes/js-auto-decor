		...
		<ul>
			...
			if ( ! empty( $args['links'] ) && is_array( $args['links'] ) ) {
				foreach ( $args['links'] as $link ) {
					...
					if ( empty( $link['social_item'] ) || empty( $link['title'] ) ) {
						continue;
					}

					// Skip if we don't have a URL pattern for this social network.
					if ( empty( $URLs[ $link['social_item'] ] ) ) {
						continue;
					}
					?>
							<li class="sp-sharing-item <?php echo esc_attr( $link['social_class'] ); ?>">
								<a href="<?php echo esc_url( $URLs[ $link['social_item'] ] ); ?>">
								<?php echo sp_kses_post( sp_render_icon( $link['icon'] ?? '', array( 'aria-hidden' => 'true' ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?>
