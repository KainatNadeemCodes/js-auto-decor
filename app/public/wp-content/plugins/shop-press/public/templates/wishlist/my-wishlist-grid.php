	...
	foreach ( $product_ids as $product_id => $product_data ) :
		...
		<div class="sp-my-wishlist-item sp-wishlist sp-product" data-product_id="<?php echo esc_attr( $product_id ); ?>" data-wishlist_key="<?php echo esc_attr( $wishlist_key ); ?>">
			<div class="sp-my-wishlist-item-thumbnail">
				<a href="<?php echo esc_url( $product->get_permalink() ); ?>"><?php echo wp_kses_post( $product->get_image( $product_id ) ); ?></a>
			</div>
			...
			<div class="sp-flex-row">
				...
				<?php if ( ! empty( $rating ) ) : ?>
					<div class="sp-my-wishlist-item-rating">
						<?php echo sp_kses_post( sp_get_svg_icon( 'star' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?>
						<?php echo wp_kses_post( $rating ); ?>
					</div>
				<?php endif; ?>
			</div>
			<div class="sp-my-wishlist-item-date-added">
				<?php echo sp_kses_post( sp_get_svg_icon( 'date' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?>
				<?php
				/* translators: %s: formatted date */
				echo wp_kses_post( sprintf( __( 'Added: %s', 'shop-press' ), esc_html( isset( $product_data['date_added'] ) ? date_i18n( $date_format, $product_data['date_added'] ) : '' ) ) );
				?>
			</div>
			...
			<div class="sp-my-wishlist-item-remove-item">
				<i class="sp-rmf-wishlist">
					<?php echo sp_kses_post( sp_get_svg_icon( 'close' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?>
				</i>
			</div>
		</div>
	<?php endforeach; ?>
