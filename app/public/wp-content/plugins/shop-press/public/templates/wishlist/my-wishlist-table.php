	...
	<thead>
		<tr>
			<?php if ( isset( $fields_in_table['thumbnail'] ) || isset( $fields_in_table['title'] ) ) { ?>
				...
				<th class="sp-h-product" colspan="3"><?php echo $custom_label_item ? esc_html( $custom_label_item ) : esc_html( __( 'Item', 'shop-press' ) ); ?></th>
			<?php } ?>
			<?php if ( isset( $fields_in_table['price'] ) ) { ?>
				<th class="sp-h-price"><?php echo $fields_in_table['price'] ? esc_html( $fields_in_table['price'] ) : esc_html( __( 'price', 'shop-press' ) ); ?></th>
			<?php } ?>
			<?php if ( isset( $fields_in_table['date_added'] ) ) { ?>
				<th class="sp-h-date-added"><?php echo $fields_in_table['date_added'] ? esc_html( $fields_in_table['date_added'] ) : esc_html( __( 'Date Added', 'shop-press' ) ); ?></th>
			<?php } ?>
			<?php if ( isset( $fields_in_table['stock'] ) ) { ?>
				<th class="sp-h-stock"><?php echo $fields_in_table['stock'] ? esc_html( $fields_in_table['stock'] ) : esc_html( __( 'stock', 'shop-press' ) ); ?></th>
			<?php } ?>
			<?php if ( isset( $fields_in_table['add_to_cart'] ) ) { ?>
				<th class="sp-h-add-to-cart"><?php echo $fields_in_table['add_to_cart'] ? esc_html( $fields_in_table['add_to_cart'] ) : ''; ?></th>
			<?php } ?>
			<?php if ( isset( $fields_in_table['remove'] ) ) { ?>
				<th class="sp-h-actions"><?php echo $fields_in_table['remove'] ? esc_html( $fields_in_table['remove'] ) : ''; ?></th>
			<?php } ?>
		</tr>
	</thead>
	<tbody>
					...
					<td class="sp-product-stock-wrapper">
						<p class="sp-product-stock <?php echo esc_attr( $product->get_stock_status() ); ?>">
							...
							if ( 'outofstock' == $product->get_stock_status() ) {
								echo esc_html( __( 'out of stock', 'shop-press' ) );
							} elseif ( 'instock' == $product->get_stock_status() ) {
								echo esc_html( __( 'in stock', 'shop-press' ) );
							}
							?>
						</p>
					</td>
