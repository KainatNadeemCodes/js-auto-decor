...
// Ensure we always have an array to avoid notices when keys are missing.
if ( ! is_array( $wishlist ) ) {
	$wishlist = array();
}

// Fallback wishlist key and ID safely without triggering undefined index notices.
$wishlist_key = $wishlist_key ? $wishlist_key : ( $wishlist['key'] ?? '' );
$wishlist_id  = isset( $wishlist['wishlist_id'] ) ? (int) $wishlist['wishlist_id'] : 0;
...
<div class="sp-my-wishlist">
	...
	if ( ! empty( $product_ids ) ) {
		...
		<div class="sp-my-wishlist-empty">
			<i class="ti-heart"></i>
			<h2><?php echo esc_html( $empty_wishlist_title ? $empty_wishlist_title : __( 'Your wishlist is currently empty', 'shop-press' ) ); ?></h2>
			<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="button return-to-shop"><?php echo esc_html( $empty_wishlist_button_text ? $empty_wishlist_button_text : __( 'Shop Now', 'shop-press' ) ); ?></a>
		</div>
		<?php
	}
