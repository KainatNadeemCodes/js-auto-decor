...
if ( empty( $products ) ) {
	?>
		<div class="sp-compare-empty">
			<h2><?php echo esc_html( __( 'Your compare list is empty', 'shop-press' ) ); ?></h2>
			<a href="<?php echo esc_url( wc_get_page_permalink( 'shop' ) ); ?>" class="button return-to-shop"><?php echo esc_html( __( 'Shop Now', 'shop-press' ) ); ?></a>
		</div>
	<?php
}
...
if ( $product ) {
	...
	foreach ( $products as $product ) {
		...
		if ( in_array( 'ratings', $fields_in_table ) ) {
			...
			$rating_html       = ! empty( $rating_html ) ? $rating_html : '<div class="star-rating" title="' . esc_attr( sprintf( __( 'Rated %s out of 5', 'shop-press' ), $rating ) ) . '">';
			$rr[ $product_id ] = array(
		...
		<table>
			<?php foreach ( $rows as $row_id ) : ?>
				<tr class="<?php echo esc_attr( $row_id ); ?>">
					...
					<?php foreach ( $r as $product_id => $rows ) : ?>
						<td class="<?php echo esc_attr( "sp-c-pr-{$product_id}" ); ?>"><?php echo 'c' !== $row_id ? wp_kses_post( $rows[ $row_id ] ) : $rows[ $row_id ]; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- SVG allowed in remove button. ?></td>
					<?php endforeach; ?>
				</tr>
			<?php endforeach; ?>
			...
			if ( $rrr_rows ) {
				...
				foreach ( $rrr_rows as $rrr_rows_id ) :
					?>
					<tr class="<?php echo esc_attr( $rrr_rows_id ); ?>">
						<td></td>
