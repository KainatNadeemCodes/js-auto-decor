...
if ( $carousel == 'true' ) {
	...
	$slider_attrs .= ' data-slick="' . esc_attr( wp_json_encode( $slider_options ) ) . '"';
...
<div class="sp-brands sp-brands-attrs sp-brands-grid">
	<div class="sp-brands-items <?php echo esc_attr( implode( ' ', $classes ) ); ?>" <?php echo $slider_attrs; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Slider attributes. ?>>
		<?php
			echo wp_kses_post( Frontend::get_brand_attributes_output( $brands, $attributes['slug'], $display_name, $display_logo ) );
		?>
	</div>
</div>
