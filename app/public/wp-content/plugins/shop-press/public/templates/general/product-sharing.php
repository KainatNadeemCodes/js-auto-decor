	...
	<div class="sp-sharing-dropdown">
		<ul>
			...
			foreach ( $share_url_patterns as $social_id => $social ) {
				...
				if ( ! in_array( $social_id, $args['links'] ) ) {
					continue;
				}
				...
				?>
						<li class="sp-sharing-item sp-sharing-item-<?php echo esc_attr( $social_id ); ?>">
							<a href="<?php echo esc_url( $social_share_link ); ?>">
						<?php echo sp_kses_post( $social['icon'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- HTML sanitized via sp_kses_post(). ?>
