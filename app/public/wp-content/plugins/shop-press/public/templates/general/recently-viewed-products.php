...
if ( 'yes' === get_option( 'woocommerce_hide_out_of_stock_items' ) ) {
	// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- Required for hiding out of stock products.
...
if ( $custom_heading ) {
	?>
	<<?php echo sp_whitelist_html_tags( $html_tag, 'h4' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> class="sp-products-heading">
		<?php echo esc_html( $products_heading ); ?>
	</<?php echo sp_whitelist_html_tags( $html_tag, 'h4' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	<?php
}
...
if ( $r->have_posts() ) {
	...
	?>
	<div class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>" <?php echo $attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Data attributes. ?>>
	...
	echo esc_html( __( 'No products found', 'shop-press' ) );
}
