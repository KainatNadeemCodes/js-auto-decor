...
?>
<div id="<?php echo esc_attr( $wrapper_id ); ?>" class="sp-product-thumbnail <?php echo esc_attr( $classes ); ?>">
	<a href="<?php echo esc_url( get_the_permalink() ); ?>">
		<?php
		echo $is_slider ? '<div class="sp-slider sp-slider-style sp-thmb-slider" data-slick="' . esc_attr( wp_json_encode( $slider_attrs ) ) . '">' : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Safe markup.
		if ( $thumb_size ) {
			// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- ThumbGen outputs safe image HTML.
			...
			if ( $is_slider ) {
				...
				foreach ( $gallery as $image_id ) {
					...
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- ThumbGen outputs safe image HTML, $thumb_size is size param.
					echo ThumbGen::instance()->image_resize_output( $image_id, $thumb_size );
				}
				...
				<div class="sp-product-th-gallery">
					<?php
					// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- ThumbGen outputs safe image HTML, $thumb_size is size param.
					echo ThumbGen::instance()->image_resize_output( $gallery[0], $thumb_size );
					?>
				</div>
				<?php
			}
		...
		echo $is_slider ? '</div>' : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Safe markup.
		?>
	</a>
	<?php
	if ( $is_slider && 'images_nav' === $nav_type && $gallery && $has_gallery_slider ) {

		echo '<div class="sp-slider sp-nav-slider sp-slider-style" data-slick="' . esc_attr( wp_json_encode( $nav_attrs ) ) . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Safe markup.
