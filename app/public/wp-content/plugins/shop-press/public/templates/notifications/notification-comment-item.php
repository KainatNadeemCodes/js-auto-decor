					...
					<div class="sp-notification-order-items">
						<div class="sp-notification-order-items-header">
							...
							foreach ( $order_items as $item ) {
								...
								$tooltip = '<span class="tooltiptext">' . esc_html( __( 'Click to submit your rate.', 'shop-press' ) ) . '</span>';
								echo '<div class="sp-notification-order-item">'
										. wp_kses_post( $featured_image )
										. '<a href="' . esc_url( $review_link ) . '" class="tooltip">' . wp_kses_post( $product_title . $tooltip ) . '</a>'
										. ( $is_review_rating_enabled ? wp_kses_post( wc_get_rating_html( $product->get_average_rating() ) ) : '' )
									. '</div>';
							}
							?>
						</table>
