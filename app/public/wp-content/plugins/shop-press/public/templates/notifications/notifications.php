...
<div class="sp-notifications-wrap">
	...
	if ( $page_count > 1 ) {
		...
		if ( 1 < $paged ) {
			...
			echo '<a class="button" href="' . esc_url( $url ) . '">' . esc_html( __( 'Prev', 'shop-press' ) ) . '</a>';
		}

			$page_url = wc_get_endpoint_url( $endpoint, 'page_number' );
			echo '<input type="number" name="" class="" placeholder="' . esc_attr__( 'Page Number', 'shop-press' ) . '" value="' . esc_attr( $paged ) . '" data-page-url="' . esc_url( $page_url ) . '" max="' . esc_attr( $page_count ) . '" onchange="window.location = jQuery(this).data(\'page-url\').replace(\'page_number\', jQuery(this).val())"/>';

		if ( $page_count > $paged ) {
			$next = $paged + 1;
			$url  = wc_get_endpoint_url( $endpoint, $next );
			echo '<a class="button" href="' . esc_url( $url ) . '">' . esc_html( __( 'Next', 'shop-press' ) ) . '</a>';
		}
			echo '</div>';
	}
	?>
