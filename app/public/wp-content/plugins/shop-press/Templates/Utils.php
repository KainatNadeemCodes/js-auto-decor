...
class Utils {
	...
	public static function render_icon( $icon, $attrs = array() ) {
		...
		if ( isset( $icon['library'] ) && 'svg' === $icon['library'] ) {
			...
			echo '<i aria-hidden="true" class="sp-icon ' . esc_attr( $icon['value'] ) . '"></i>';
		}
	...
	public static function AddToCartButton( $product, $icon = '', $icon_position = 'after', $show_text = true, $custom_text = '' ) {
		...
		if ( $is_single ) {
			...
			if ( 'variable' == $product->get_type() || 'grouped' == $product->get_type() ) {
				...
				echo '<button type="submit" class="single_add_to_cart_button button alt' . ( esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ) ) . ' ' . esc_attr( $classes ) . '">' . wp_kses_post( $before ) . esc_html( $text ) . wp_kses_post( $after ) . '</button>';
			} else {

				echo '<button type="submit" name="add-to-cart" value="' . esc_attr( $product->get_id() ) . '" class="single_add_to_cart_button button alt' . ( esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ) ) . ' ' . esc_attr( $classes ) . '">' . wp_kses_post( $before ) . esc_html( $text ) . wp_kses_post( $after ) . '</button>';
			}
		}
	}
