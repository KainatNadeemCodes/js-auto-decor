...
class ProductCollection {
	...
	public function prepare_query_args( $query_args, $attributes, $type ) {
		...
		$orderby = isset( $_REQUEST['orderby'] ) && ! empty( $_REQUEST['orderby'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['orderby'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Product sorting.
		if ( ! empty( $orderby ) ) {
	...
	public function render() {
		...
		// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Data attributes and slider attrs built safely.
		echo '<div id="sp-products-loop-' . esc_attr( wp_rand() ) . '" class="sp-products-loop-wrapper ' . esc_attr( $classes ) . ' ' . esc_attr( $this->get_slider_class() ) . '" ' . $data_attributes . ' ' . $this->get_slider_attrs() . ' >';
