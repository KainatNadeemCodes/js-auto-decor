...
class ProductShop {
	...
	public function display_load_more( $attrs ) {
		...
		if ( $load_more_button ) {
			...
			echo '<a href="#" class="sp-loadmore">' . wp_kses_post( $btn_html ) . '</a>';
			echo '</div>';
		}
	...
	private function is_edit_screen() {
		$is_block_editor     = isset( $_GET['_locale'] ) && sanitize_text_field( wp_unslash( $_GET['_locale'] ) ) === 'user'; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Editor context check.
