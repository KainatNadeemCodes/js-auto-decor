...
class QuickView {
	...
	public static function enqueue() {
		...
		if ( ( is_page() || is_woocommerce() || is_tax( 'product_brand' ) ) && ! is_checkout() ) {
	...
	public static function essensial_scripts() {
		// phpcs:ignore WordPress.WP.EnqueuedResources.NonEnqueuedScript -- Quick view modal loads scripts inline for AJAX context.
	...
	public static function title() {
		?>
			<h3 class="sp-qv-title"><?php echo esc_html( get_the_title() ); ?></h3>
		<?php
	}
	...
	public static function is_quick_view_ajax() {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- Request type detection only; nonce verified in quick_view_ajax handler.
		return is_ajax() && isset( $_POST['action'] ) && 'quick_view_ajax' === sanitize_text_field( wp_unslash( $_POST['action'] ) );
	}
