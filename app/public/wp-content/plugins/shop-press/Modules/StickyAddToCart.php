		...
		<div class="sp-sticky-add-to-cart <?php echo $hide_on_mobile ? 'sp-hide-on-mobile' : ''; ?>">
			...
			<div class="sticky-add-to-cart-btn product">
				...
				if ( $product->is_type( 'simple' ) ) {
					...
					echo '<a href="' . esc_url( $product->add_to_cart_url() ) . '" class="single_add_to_cart_button sp-sticky-scroll-add-to-cart button alt">' . ( true == $product->is_type( 'variable' ) || true == $product->is_type( 'grouped' ) ? esc_html__( 'Select Options', 'shop-press' ) : esc_html( $product->single_add_to_cart_text() ) ) . '</a>';
				}
				?>
			</div>
		</div>
