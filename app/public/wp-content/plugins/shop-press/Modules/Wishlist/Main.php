...
class Main {
	...
	public static function get_current_user_wishlist_products_in_cookie() {
		...
		// JSON cookie value - wp_unslash needed; output validated via is_array().
		// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON cannot use sanitize_text_field
		$decoded = json_decode( wp_unslash( $_COOKIE['shoppress_wishlist'] ), true );
	...
	public static function add_to_wishlist_by_ajax() {
		...
		$wishlist_key = isset( $_POST['wishlist_key'] ) ? sanitize_text_field( wp_unslash( $_POST['wishlist_key'] ) ) : false;
		if ( ! $wishlist_key || ! $multi_wishlist_status ) {
			...
			$wishlist_title        = isset( $_POST['wishlist_title'] ) ? sanitize_text_field( wp_unslash( $_POST['wishlist_title'] ) ) : false;
			$wishlist_share_status = 'public';
			$wishlist_atts         = array();
			if ( ! $user_id ) {
				$wishlist_atts['author_name']  = isset( $_POST['author_name'] ) ? sanitize_text_field( wp_unslash( $_POST['author_name'] ) ) : false;
				$wishlist_atts['author_email'] = isset( $_POST['author_email'] ) ? sanitize_email( wp_unslash( $_POST['author_email'] ) ) : false;
			} else {

				$wishlist_share_status = isset( $_POST['wishlist_share_status'] ) ? sanitize_text_field( wp_unslash( $_POST['wishlist_share_status'] ) ) : 'private';
			}
	...
	public static function run_action_by_ajax() {
		...
		check_ajax_referer( 'shoppress_nonce', 'nonce' );
		$action      = isset( $_POST['action2'] ) ? sanitize_text_field( wp_unslash( $_POST['action2'] ) ) : false;
		...
		switch ( $action ) {
			case 'move_to_another_wishlist':
				$move_to              = isset( $_POST['move_to'] ) ? sanitize_text_field( wp_unslash( $_POST['move_to'] ) ) : '';
				$current_wishlist_key = isset( $_POST['current_wishlist_key'] ) ? sanitize_text_field( wp_unslash( $_POST['current_wishlist_key'] ) ) : '';
