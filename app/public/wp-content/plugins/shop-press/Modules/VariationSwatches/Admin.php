...
class Admin {
	...
	public function admin_enqueue( $hook ) {
		...
		wp_enqueue_style( 'sp-admin-pickr-style', SHOPPRESS_URL . 'public/modules/variation-swatches/admin/css/sp-pickr.css', array(), SHOPPRESS_VERSION );
		wp_enqueue_style( 'sp-admin-style', SHOPPRESS_URL . 'public/modules/variation-swatches/admin/css/sp-admin.css', array(), SHOPPRESS_VERSION );
				...
				<div class='sp-upload-image'>
					...
					<div style="line-height:60px;">
						...
						<button type="button" class="sp-upload-image-button button " onclick="sp_vs_upload_icon_image( this,event )">
							<span class="sp-upload-button"><?php echo esc_html( __( 'Upload', 'shop-press' ) ); ?></span>
						</button>

						<button type="button" style="display:none" class="sp_vs_remove_image_button button " onclick="sp_vs_remove_icon_image( this,event )">
							<span class="sp-remove-button"><?php echo esc_html( __( 'Remove', 'shop-press' ) ); ?></span>
						</button>
					</div>
				</div>
			</div>
	...
	public function save_term_meta( $term_id, $tt_id, $taxonomy ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- WordPress term edit form verifies nonce.
		$product_key = 'product_' . $taxonomy;
		if ( isset( $_POST[ $product_key ] ) && ! empty( $_POST[ $product_key ] ) ) {
			update_term_meta( $term_id, $product_key, wc_clean( wp_unslash( $_POST[ $product_key ] ) ) );
		} else {
	...
	public function add_attribute_column( $columns ) {
		$new_columns = array();
		// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Admin list table context.
	...
	public function product_option_terms( $attribute_taxonomy, $i ) {
		...
		if ( 'select' !== $attribute_taxonomy->attribute_type ) {
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Missing -- WooCommerce product screen verifies nonce.
			...
			<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'shop-press' ); ?>" class="multiselect attribute_values wc-enhanced-select" name="attribute_values[<?php echo esc_attr( $i ); ?>][]">
				<?php
					...
					$all_terms = get_terms( array_merge( array( 'taxonomy' => $taxonomy ), apply_filters( 'woocommerce_product_attribute_terms', $args ) ) );
					...
					if ( $all_terms ) :
						...
						foreach ( $all_terms as $term ) :
							...
							echo '<option value="' . esc_attr( $term->term_id ) . '" ' . esc_attr( wc_selected( has_term( absint( $term->term_id ), $taxonomy, $product_id ), true, false ) ) . '>' . esc_html( apply_filters( 'woocommerce_product_attribute_term_name', $term->name, $term ) ) . '</option>';
						endforeach;
					endif;
					?>
			</select>
			...
			if ( ( $attribute_taxonomy->attribute_type == 'label' || $attribute_taxonomy->attribute_type == 'image' || $attribute_taxonomy->attribute_type == 'color' ) ) {
				?>
				<?php /* translators: %s: attribute label */ ?>
				<button class="button fr plus sp_vs_add_new_attribute"  data-attr_taxonomy="<?php echo esc_attr( $taxonomy ); ?>"  data-attr_type="<?php echo esc_attr( $attr_type ); ?>"  data-dialog_title="<?php echo esc_attr( sprintf( __( 'Add new %s', 'shop-press' ), $attribute_taxonomy->attribute_label ) ); ?>">
			...
			<div id="custom_variations_inner">
				...
				if ( ! $has_custom_attribute ) {
					...
					<div class="inline notice woocommerce-message">
						...
						<p>
						<?php
						esc_html_e( 'No custom attributes added yet.', 'shop-press' );
						esc_html_e( ' You can add custom attributes from the', 'shop-press' );
						?>
						<a onclick="spTriggerAttributeTab( this )" href="#woocommerce-product-data"><?php esc_html_e( ' Attributes', 'shop-press' ); ?> </a> <?php esc_html_e( 'tab', 'shop-press' ); ?></p>
					</div>
					<?php
				}
				?>
				...
				<tr class="sp-term-name">
					<td colspan="2">
						...
						<table class="sp-local-body-table">
							<tbody class="sp-local-body sp-local-body-<?php echo esc_attr( $term ); ?>" style="<?php echo esc_attr( $css ); ?>">
								<tr>
									<td width="30%"><?php esc_html_e( 'Term Name', 'shop-press' ); ?></td>
									<td width="70%"><?php echo esc_html( $term_title ); ?></td>
								</tr>
								<tr class="form-field"> <td><?php esc_html_e( 'Term Image', 'shop-press' ); ?></td>
									<td>
									<?php
										...
										<div class ="sp-upload-image">
											<div class="tawcvs-term-image-thumbnail" style="float:left;margin-right:10px;">
												...
												<div style="line-height:30px;">
													<?php
													// Ensure we always store a scalar ID, avoid array-to-string notices.
													if ( is_array( $term_field ) ) {
														$term_field = reset( $term_field );
													}
													?>
													<input type="hidden" class="i_index_media"  name= "<?php echo esc_attr( sanitize_title( 'image_' . $attribute_name . '_term_' . $term ) ); ?>" value="<?php echo esc_attr( (string) $term_field ); ?>">
													<button type="button" class="sp-upload-image-button button " onclick="sp_vs_upload_icon_image( this,event )">
														<span class="sp-upload-button"><?php echo esc_html( __( 'Upload', 'shop-press' ) ); ?></span>
													</button>
													<button type="button" style="<?php echo esc_attr( $remove_img ); ?>" class="sp_vs_remove_image_button button " onclick="sp_vs_remove_icon_image( this,event )">
														<span class="sp-remove-button"><?php echo esc_html( __( 'Remove', 'shop-press' ) ); ?></span>
													</button>
												</div>
											</div>
	...
	public function output_field_color( $type, $attribute, $post_id ) {
		...
		$is_taxonomy    = false !== $attribute->is_taxonomy() ? true : false;
		?>
		<table class="sp-custom-table sp-custom-table-color" style="<?php echo esc_attr( $display_status ); ?>">
			<?php
			$i = 0;
			foreach ( $attribute->get_options() as $term ) {
				$term_object = $is_taxonomy ? get_term( $term ) : false;
	...
	public function save_custom_fields( $post_id, $post ) {
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- WooCommerce product save verifies nonce.
		...
		foreach ( $product->get_attributes() as $attribute ) {
			...
			if ( $attr_settings['type'] == 'radio' ) {
				...
				foreach ( $attribute->get_options() as $term ) {
					...
					if ( $attr_settings['type'] == 'color' ) {
						$term_value = isset( $_POST[ $term_key ] ) ? array_map( 'sanitize_text_field', array_map( 'wp_unslash', (array) $_POST[ $term_key ] ) ) : array();
					} else {
						$term_value = isset( $_POST[ $term_key ] ) ? sanitize_text_field( wp_unslash( $_POST[ $term_key ] ) ) : '';
					}
