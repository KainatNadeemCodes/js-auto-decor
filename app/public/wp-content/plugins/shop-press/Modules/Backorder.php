...
class Backorder {
	...
	public static function set_order_limit_single( $passed, $product_id, $quantity ) {
		...
		if ( $cart_items_count >= $limit || $total_count > $limit ) {
			...
			/* translators: %d: available limit number */
			wc_add_notice( sprintf( __( 'The maximum backorder limit has reached. (%d available)', 'shop-press' ), $limit ), 'error' );
		}
	...
	public static function add_fields() {
		...
		?>
		<div class="sp-backorder-fields <?php echo ( $display_backorder && 'onbackorder' === $stock_status ) ? esc_attr( 'sp-backorder-fields-show' ) : ''; ?>">
		<?php

		$field = array(
			'id'          => 'sp_backorder_limit',
			'value'       => $backorder_limit,
			/* translators: %d: default limit number */
			'placeholder' => sprintf( __( 'Default Limit (%d)', 'shop-press' ), $limit ),
		...
		?>
			<p class="form-field sp_backorder_date_field ">
				...
				<input type="date" name="sp_backorder_date" id="sp_backorder_date" value="<?php echo esc_attr( $backorder_date ); ?>">
			</p>
		</div>
		<?php
	}
	...
	public static function save_meta_box( $post_id ) {
		...
		if ( ! isset( $_REQUEST['woocommerce_meta_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['woocommerce_meta_nonce'] ) ), 'woocommerce_save_data' ) ) {
			return;
		}
		if ( isset( $_REQUEST['sp_backorder_limit'] ) ) {
			update_post_meta( $post_id, 'sp_backorder_limit', sanitize_text_field( wp_unslash( $_REQUEST['sp_backorder_limit'] ) ) );
		}

		if ( isset( $_REQUEST['sp_backorder_date'] ) ) {
			update_post_meta( $post_id, 'sp_backorder_date', sanitize_text_field( wp_unslash( $_REQUEST['sp_backorder_date'] ) ) );
		}
	}
}
