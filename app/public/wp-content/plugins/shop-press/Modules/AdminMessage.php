...
class AdminMessage {
		...
		<div class="shoppress-plugin-requires sp-go-pro" data-message="pro">
			...
			<div class="box-content">
				<div>
					...
					<div class="box-install">
						<a href="https://webnus.net/shoppress/">Go Premium</a>
					</div>
				</div>
			</div>
	...
	public function shoppress_admin_message() {
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'shoppress_message' ) ) {
			die( 'Wrong nonce!' );
		}

		$msgData = isset( $_POST['msgData'] ) ? sanitize_text_field( wp_unslash( $_POST['msgData'] ) ) : '';
		$msgPro  = isset( $_POST['msgPro'] ) ? sanitize_text_field( wp_unslash( $_POST['msgPro'] ) ) : '';
