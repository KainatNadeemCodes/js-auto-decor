...
class Compare {
	...
	public static function add_to_compare_by_ajax() {
		...
		$Product = sanitize_text_field( wp_unslash( $_POST['product_id'] ) );
	...
	public static function get_compare_product_ids( $limit = -1 ) {
		...
		if ( $user_id ) {
			...
			if ( ! isset( $_COOKIE['shoppress_woo_compare'] ) ) {
				...
				// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- JSON cookie, output validated via is_array
				$Compare = json_decode( wp_unslash( $_COOKIE['shoppress_woo_compare'] ), true );
				$Compare = is_array( $Compare ) ? $Compare : false;
			}
		}
	...
	public static function compare_popup_output() {
		ob_start();
			echo '<div class="sp-compare-popup shoppress-compare-popup-wrap">
				...
				echo wp_kses_post( self::render_compare_page() );
