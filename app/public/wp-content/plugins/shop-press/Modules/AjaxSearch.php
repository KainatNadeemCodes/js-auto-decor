...
class AjaxSearch {
	...
	public static function sp_ajax_search() {
		...
		$res   = isset( $_POST['res'] ) ? sanitize_text_field( wp_unslash( $_POST['res'] ) ) : '';
	...
	public static function search( $s, $cat, $limit, $res ) {
		...
		if ( ! empty( $cat ) ) {
			...
			// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query -- Product category filter required.
